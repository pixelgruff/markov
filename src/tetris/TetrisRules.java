...
public class TetrisRules implements Rules<TetrisState, TetrisAction>
{
    ...
    @Override
    public TetrisState filterState(final TetrisState state, final Player p)
    {
        throw new UnsupportedOperationException("Not supported yet.");
    }
    ...
    /**
     * Lower all the 'falling' Tetriminos in the state. Handles collisions
     * without moving pieces into invalid states, but does not trigger terminal
     * conditions, scoring, row removal, merging.
     *
     * @param state
     */
    private void dropTetriminos(final TetrisState state)
    {
        ...
        state.getAllShapes().stream().filter((shape) -> (!shape.isOnFloor()))
    ...
    {
        return new HashSet<TetrisShape>(state.getAllShapes().stream().filter(topShape
        ...
        && topShape.getAllBlocks().stream().anyMatch(topBlock -> bottomShape.getAllBlocks()
        /* With the top shape's block directly above the bottom shape's block */
        .contains(topBlock.add(new Vector2(0, -1)))))).collect(Collectors.toSet()));
    }

    @Override
    public Map<Player, Score> scores(final TetrisState state)
    {
